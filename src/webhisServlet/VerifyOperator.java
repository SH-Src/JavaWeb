...
import java.sql.*;
public class VerifyOperator extends HttpServlet{
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        try{
            ...
            if(rs.next() && rs.getString("password").equals(pwd)) {
                request.setAttribute("operatorID",rs.getInt("id"));
                request.setAttribute("operatorName",rs.getString("realname"));
                request.getRequestDispatcher("/charge.jsp").forward(request, response);
            }else {
                request.setAttribute("message","登录失败！");
                request.getRequestDispatcher("/operator-login.jsp").forward(request, response);
            }
            rs.close();
            stmt.close();
            conn.close();
